...
API_URL = LOCAL_URL  # CLOUD_URL | LOCAL_URL - Default to LOCAL_URL for local development
...
def set_overlay_session_states(raw_images, segmented_images):
    ...
    # raw_images are numpy arrays with shape (512, 512, 3) and dtype uint8
    if isinstance(raw_images[0], np.ndarray):
        st.session_state.start_sat = Image.fromarray(raw_images[0])
        st.session_state.end_sat = Image.fromarray(raw_images[-1])
    else:
...
def process_calculation_request(latitude, longitude, start_date, end_date):
    # Convert dates to strings if they are datetime.date or datetime.datetime objects
...
def fill_sidebar_top_col2():
    ...
    if st.button(
        ...
        # Use the original API-based calculation for accurate deforestation percentage
...
def fill_sidebar_bot_col1():
    with st.container(border=True):
        ...
        if st.button('Calculate   ', use_container_width=True, type='primary'):
            ...
            st.session_state.longitude_input = -59.72
            st.session_state.zoom = 12.5
            st.session_state.start_timeframe = dt.datetime(2017, 8, 24).date()
            st.session_state.end_timeframe = dt.datetime(2024, 4, 24).date()
            process_calculation_request(
                latitude=st.session_state.latitude_input,
                longitude=st.session_state.longitude_input,
                start_date=st.session_state.start_timeframe,
...
st.sidebar.image(LOGO_URL, width=True)
...
if st.session_state.get("show_intervall_analytics", False):
    try:
        ...
        st.error(f"Error processing satellite images: {str(e)}")
        st.info("Please try again with different coordinates or date range.")
        st.session_state.show_intervall_analytics = False
        image_dates, segmented_images, raw_images = None, None, None
    
    if image_dates is not None and segmented_images is not None and raw_images is not None:
        ...
        with st.container(border=False):
            ...
            with st.container(border=True):
                ...
                for col, date, raw_image, segmented_image in zip(
                    ...
                    col.image(raw_image, width="content")
                    col.image(
                        bw_to_color(

...
def load_images_from_satellite(
    ...
    for date_i in date_list:
        # Try with strict cloud coverage first (10%)
        optimal_tile = search_optimal_l2a_tiles(
            catalog=catalog_,
            bbox=box,
            date_request=date_i,
            range_days=180,  # Increased from 91 to 180 days (6 months)
            max_cloud_coverage=10
        )
        
        # If no results, try with relaxed cloud coverage (50%)
        if not optimal_tile:
            print(f"No images found for {date_i} with 10% cloud coverage, trying 50%...")
            optimal_tile = search_optimal_l2a_tiles(
                catalog=catalog_,
                bbox=box,
                date_request=date_i,
        ...
        if optimal_tile:
           ...
           print(f"Found image for {date_i}: {optimal_tile.get('date')} (cloud cover: {optimal_tile.get('cloud_cover')}%)")
        else:
           print(f"WARNING: No suitable image found for date {date_i} even with relaxed criteria")

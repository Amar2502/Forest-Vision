...
def create_sentinelhub_token() -> Tuple[SHConfig, SentinelHubCatalog]:
    ...
    """
    config = SHConfig()
    config.sh_client_id = "01437ac0-0706-41d8-a085-48825d8dc0b4"
    config.sh_client_secret = "RxXFeZdwYhAmcmWrEse3If7xceG55uKs"
    ...
    if not config.sh_client_id or not config.sh_client_secret:
        ...
        print(f"CLIENT_ID found: {bool(config.sh_client_id)}")
        print(f"CLIENT_SECRET found: {bool(config.sh_client_secret)}")
        print("Make sure to set SENTINEL_CLIENT_ID and SENTINEL_CLIENT_SECRET environment variables")
...
def search_optimal_l2a_tiles(
    ...
    print(f"Searching for images near date: {date_request}")
    print(f"Time interval: {time_interval}")
    print(f"BBox: {bbox}")
    print(f"Max cloud coverage: {max_cloud_coverage}%")
    ...
    try:
        ...
        print(f"Found {len(results)} images with cloud coverage < {max_cloud_coverage}%")
        
        # If no results with strict cloud coverage, try with relaxed coverage
        if not results and max_cloud_coverage < 50:
            print(f"No images found with {max_cloud_coverage}% cloud coverage. Trying with 50%...")
            results = list(catalog.search(
                DataCollection.SENTINEL2_L2A,
                bbox=bbox,
                time=time_interval,
            ...
            print(f"Found {len(results)} images with cloud coverage < 50%")
        
        if not results:
            print("No images found in the specified time range and location.")
            return None
    except Exception as e:
        print(f"Error searching for images: {e}")
        print("This might be due to missing or invalid SentinelHub credentials.")
        return None
    properties = []
    for tile in results:
